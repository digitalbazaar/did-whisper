...
async function encryptForDid(did, message, expiration) {
  const cipherKeys = await cipherKeysForDid({did, location: 'all', mode: 'test'});
...
async function decryptForDid(did, cipher, keyId) {
  const recipientKeys = await cipherKeysForDid({did, location: 'local', mode: 'test'});
...
async function cipherKeysForDid(options, keyId) {
  ...
  if(!doc) {
    throw new Error(`Could not fetch local DID Document for ${options.did}`);
  }
